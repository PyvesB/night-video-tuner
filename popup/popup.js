...
Object.keys(FILTERS).forEach(
		function(filter) {
			...
			document.getElementById(filter + "_text").appendChild(
					document.createTextNode(chrome.i18n.getMessage(messageName)));
			...
			chrome.storage.local.get(filter, function(value) {
				...
				if (typeof storageValue !== "undefined") {
					...
					var filterVal = document.getElementById(filter + "_val");
...
chrome.storage.local.get("state", function(value) {
	var buttonText = document.getElementById("enable_disable_text");
